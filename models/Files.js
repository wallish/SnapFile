...
Files.prototype.select = function (folder, cb){
	...
	mysql.getConnection(function(connectionerr, connection) {
		connection.query('SELECT fi.id, fi.idFolder, fi.originalName, fi.creator, fi.type, fi.downloadCount, fi.dateinsert, fi.expire from Files fi LEFT JOIN Folders fo ON fi.idFolder=fo.id WHERE fo.name="'+folder+'" AND fi.deleted=0 AND fo.deleted=0', function (err, rows, fields) {
		  	if (err) throw err;
		 	//console.log(rows);
	 		cb(rows);
			connection.release();	
